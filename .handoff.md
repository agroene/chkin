...
**1. Added Audit Log Viewer (Admin Console)**
- New page at `/admin/audit` for POPIA compliance tracking
- APIs: GET `/api/admin/audit` (list with filters), `/api/admin/audit/filters`, `/api/admin/audit/export`
- Features: Date range filter, action/user/org/resource type filters
- CSV export with all filters applied
- Expandable rows showing user agent and metadata
- Pagination (50 per page)

**2. Added Local Startup Script**
- PowerShell script `start-local.ps1` for one-command dev environment setup
- Starts Docker PostgreSQL, waits for health, runs migrations, seeds, starts dev server
- Added `npm run start:local` shortcut

**3. Fixed Field Library "Failed to Fetch" Error**
...
6. Starts dev server at http://localhost:3000
